...
- Train simple CNN + LSTM model from scratch
- Report:
...
class SimpleCRNN(nn.Module):
    def __init__(self, n_mels=N_MELS, n_classes=N_CLASSES):
        ...
        self.conv = nn.Sequential(
            ...
            nn.ReLU(),
            nn.MaxPool2d(2),            # /2 in freq & time
            nn.Conv2d(16, 32, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(2)             # /4 total
        )
        conv_dim = (n_mels // 4) * 32   # features per time step
        self.lstm = nn.LSTM(
            input_size=conv_dim,
            hidden_size=64,
            num_layers=1,
    ...
    def forward(self, x):
        # x: (B,1,n_mels,T)
        h = self.conv(x)   # (B, C, M', T')
        B, C, M, T = h.shape
        h = h.permute(0, 3, 1, 2).contiguous().view(B, T, C * M)  # (B,T,F)
        y, _ = self.lstm(h)   # (B,T,2*hidden)
        # simple mean over time (no attention, keep it simple)
        emb = y.mean(dim=1)   # (B, 2*hidden)
        logits = self.fc(emb)
        return logits

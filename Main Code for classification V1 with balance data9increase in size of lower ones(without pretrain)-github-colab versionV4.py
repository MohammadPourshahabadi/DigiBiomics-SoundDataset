...
def main():
    ...
    # ---- Evaluate BEST checkpoint on VAL and TEST (CM + ROC/AUC + TEST METRICS) ----
    ...
    if os.path.isfile(best_path):
        ...
        pd.DataFrame([test_metrics]).to_csv(os.path.join(OUTDIR, "best_test_metrics_summary.csv"), index=False)

        test_report = classification_report(yt_test, yp_test, target_names=LABELS, output_dict=True, zero_division=0)
        pd.DataFrame(test_report).T.to_csv(os.path.join(OUTDIR, "best_test_classification_report.csv"))

        print("[Best/Test] Acc={acc:.3f}  F1(macro)={f1m:.3f}  F1(micro)={f1mi:.3f}  BalAcc={ba:.3f}  κ={kap:.3f}  MCC={mcc:.3f}".format(

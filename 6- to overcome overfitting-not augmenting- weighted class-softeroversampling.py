...
Key features:
- Group-wise Test split (~15% by samples, no patient leakage) via GroupShuffleSplit
- From remaining groups: group-wise Train/Val (~85/15)
- TRAIN set balancing with *soft oversampling*:
    each class oversampled up to max_mult * original_count (default 2.0)
...
- Confusion matrices (overall + one-vs-rest) for Val & Test
- ROC + AUC (per-class, micro, macro) for Val & Test
- Learning curves, hyperparameters table
- Test metrics CSVs: accuracy, precision/recall/F1 (micro/macro), balanced acc, kappa, MCC, per-class report
"""
...
WEIGHT_DECAY = 1e-3
TARGET_SEC = 2.5
BALANCE_DATA = True          # use soft oversampling on TRAIN ONLY
...
# Optional quick peek (can be removed)
...
# ------------------- SPLITTING & BALANCING ------------------- #
def oversample_indices_soft(base_dataset, indices, seed=SEED, out_csv=None, max_mult=2.0):
    """
    Soft oversampling:
    - For each class, oversample up to target_per_class = min(max_orig, max_mult * count_class)
    - Avoids extreme duplication of rare classes.
    """
    ...
    max_orig = max(counts.values())
    target_per_class = {
        y: int(min(max_orig, max_mult * c)) for y, c in counts.items()
    }

    expanded = []
    for y, idxs in label_to_idx.items():
        need = target_per_class[y] - len(idxs)
        if need > 0:
    ...
    if out_csv:
        ...
        print(
            f"[Oversample-soft] orig={counts}, target={target_per_class}, total={len(expanded)}"
        )
...
def groupwise_split_indices(base_dataset, test_size=0.15, val_size_within_remain=0.15, seed=SEED):
    ...
    1) Test groups via GroupShuffleSplit (~15% by samples)
    2) From remaining groups, Train/Val via GroupShuffleSplit (~15% of remain)
...
def save_hyperparams(outdir):
    ...
    df = pd.DataFrame(
        {"hyperparameter": list(HYPERPARAMS.keys()), "value": list(HYPERPARAMS.values())}
    )
...
def save_confusions(y_true, y_pred, labels, outdir, prefix):
    ...
    for i, name in enumerate(labels):
        ...
        df_i.to_csv(
            os.path.join(outdir, f"{prefix}_confusion_{name}_onevsrest.csv")
        )
...
def main():
    ...
    save_hyperparams(OUTDIR)

    # Base dataset (no aug) for splitting
    ...
    # Soft oversampling on TRAIN ONLY
    if BALANCE_DATA:
        train_idx_bal = oversample_indices_soft(
            base,
            train_idx,
            seed=SEED,
            out_csv=os.path.join(OUTDIR, "train_oversampled.csv"),
            max_mult=2.0,  # cap oversampling
        )
    else:
    ...
    # Datasets / loaders
    train_ds = Subset(
        ICBHICycleDataset(DATASET_ROOT, TARGET_SEC, True, augment=True),
        train_idx_bal,
    )
    val_ds = Subset(
        ICBHICycleDataset(DATASET_ROOT, TARGET_SEC, False, augment=False),
        val_idx.tolist(),
    )
    test_ds = Subset(
        ICBHICycleDataset(DATASET_ROOT, TARGET_SEC, False, augment=False),
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for X, y in tqdm(
            train_dl, desc=f"Training epoch {epoch}", leave=False
        ):
            if X is None:
                continue
        ...
        with torch.no_grad():
            for X, y in tqdm(
                val_dl, desc=f"Validation epoch {epoch}", leave=False
            ):
                if X is None:
                    continue
                X, y = X.to(device), y.to(device)
        ...
        yprob_val = (
            np.vstack(yprob_val_list)
            if len(yprob_val_list) > 0
        ...
        metrics = {
            ...
            "val_precision_micro": precision_score(
                yt_val, yp_val, average="micro", zero_division=0
            ),
            ...
            "val_precision_macro": precision_score(
                yt_val, yp_val, average="macro", zero_division=0
            ),
        ...
        df_rep.to_csv(
            os.path.join(
                OUTDIR, f"metrics_per_class_epoch{epoch:03d}.csv"
            ),
            index=True,
        )

        print(
            f"[Epoch {epoch}] Train Acc={tr_acc:.3f} "
        ...
        if va_mf1 > best_mf1:
            ...
            if no_improve >= PATIENCE:
                print(
                    f"No improvement for {PATIENCE} epochs → early stop."
                )
                break
    ...
    if os.path.isfile(best_path):
        ...
        with torch.no_grad():
            for X, y in tqdm(
                val_dl, desc="Best model eval (val set)"
            ):
                if X is None:
                    continue
                X = X.to(device)
                out = best_model(X)
                yprob_best_list.append(
                    torch.softmax(out, dim=1).cpu().numpy()
                )
                yp_best.extend(out.argmax(1).cpu().numpy())
                yt_best.extend(y.numpy())
        yprob_best = (
            np.vstack(yprob_best_list)
            if len(yprob_best_list) > 0
            else np.zeros((0, N_CLASSES))
        )
        save_confusions(
            yt_best, yp_best, LABELS, OUTDIR, prefix="best_val"
        )
        if len(yprob_best):
            save_multiclass_roc(
                yt_best, yprob_best, LABELS, OUTDIR, prefix="best_val"
            )
        ...
        with torch.no_grad():
            for X, y in tqdm(
                test_dl, desc="Best model eval (test set)"
            ):
                if X is None:
                    continue
                X = X.to(device)
                out = best_model(X)
                yprob_test_list.append(
                    torch.softmax(out, dim=1).cpu().numpy()
                )
                yp_test.extend(out.argmax(1).cpu().numpy())
                yt_test.extend(y.numpy())
        yprob_test = (
            np.vstack(yprob_test_list)
            if len(yprob_test_list) > 0
            else np.zeros((0, N_CLASSES))
        )

        save_confusions(
            yt_test, yp_test, LABELS, OUTDIR, prefix="best_test"
        )
        if len(yprob_test):
            save_multiclass_roc(
                yt_test,
        ...
        test_metrics = {
            ...
            "test_precision_micro": precision_score(
                yt_test, yp_test, average="micro", zero_division=0
            ),
            ...
            "test_precision_macro": precision_score(
                yt_test, yp_test, average="macro", zero_division=0
            ),
        ...
        pd.DataFrame(test_report).T.to_csv(
            os.path.join(
                OUTDIR, "best_test_classification_report.csv"
            )
        )
